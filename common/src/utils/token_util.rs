...
/// Utility struct to do some verifications on the provided mints, make sure we don't have any extensions (spl-2022) that could break the vault.
/// They could break the vault program by requiring additional accounts when doing CPIs, which could lead to transaction size issues.
...
impl TokenUtil {
    ...
    ///
    /// In the frontend we will show a warning if any mints in the vault have these extensions.
    ///
